    ...
    for i, line in enumerate(raw_bulk):
        if line[:2] == "- ":
            ...
            if content_type == "H":
                ...
                highlight_bound = {
                    list(books[book_name])[-1]: raw_bulk[i + 2][:-1]
                }  # This binds the note with the previous highlight
...
print(
    f'{PURPLE_COLOR}Please make sure you have pasted "My Clippings.txt" file in this folder!{RESET_COLOR} \n{YELLOW_COLOR}This program is sensitive to the filename... :){RESET_COLOR}'
)
    ...
    try:
        ...
        for highlight in book_highlights[book]:
            ...
            if book_highlights[book][highlight] != None:
                add_blocks_to_page(
                    book_highlights[book][highlight], existing_pages[book], is_note=True
                )
    except:
        print(
